...
class Conversation(db.Model):
    ...
    user_one_id = db.Column(db.Integer, db.ForeignKey('EMPLOYEE.Employeeid'), nullable=False)

    user_two_id = db.Column(db.Integer, db.ForeignKey('EMPLOYEE.Employeeid'), nullable=False)

    user_one = db.relationship('Employees', foreign_keys=[user_one_id])

    user_two = db.relationship('Employees', foreign_keys=[user_two_id])
...
class Message(db.Model):
    ...
    sender_id = db.Column(db.Integer, db.ForeignKey('EMPLOYEE.Employeeid'), nullable=False)

    content = db.Column(db.Text, nullable=False)

    timestamp = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    sender = db.relationship('Employees')


class ConversationRead(db.Model):

    __tablename__ = 'conversation_read'

    id = db.Column(db.Integer, primary_key=True)

    conversation_id = db.Column(db.Integer, db.ForeignKey('conversation.id'), nullable=False)

    user_id = db.Column(db.Integer, db.ForeignKey('EMPLOYEE.Employeeid'), nullable=False)

    last_read_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    conversation = db.relationship('Conversation', backref=db.backref('reads', lazy=True))

    user = db.relationship('Employees')
